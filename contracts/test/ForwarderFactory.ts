...
describe('ForwarderFactory', function () {
  async function deployFixture() {
    ...
    const ForwarderFactory =
      await hardhat.ethers.getContractFactory('ForwarderFactory');
  ...
  describe('computeAddress', function () {
    it('Should output the same result for the same salt', async function () {
      ...
      expect(
        await forwarderFactory.computeAddress(otherAccount.address, salt),
      ).to.equal(
        await forwarderFactory.computeAddress(otherAccount.address, salt),
      );
    });
  ...
  describe('createContract', function () {
    ...
    it('Should fail to create contract with the same salt and forwardTo', async function () {
      ...
      await expect(forwarderFactory.createForwarder(otherAccount.address, salt))
        .to.be.reverted;
    });
    ...
    it('Should create the contract in the computed address', async function () {
      ...
      const computedAddress = await forwarderFactory.computeAddress(
        forwardTo,
        salt,
      );
